...
int main(int argc, char* argv[]) {
	...
	srand(time(NULL));
	uintmax_t rotwidth = 20; /* HACK: pinned */
	...
	while (1) {
		...
		{ static FILE* tf = 0; static long tn = 0; static long tmax = -1;
		  if (!tf) { const char* p = getenv("TIO_TRACE"); if (p) tf = fopen(p, "w");
		             const char* m = getenv("TIO_TRACE_MAX"); tmax = m ? atol(m) : 2000000; }
		  if (tf) { fprintf(tf, "%ld %d %d\n", tn, pos, (int)((c->memptr->val->unicode+pos)%94));
		            if (++tn >= tmax) { fclose(tf); exit(42); } } }
		switch ((c->memptr->val->unicode+pos)%94) {
			case 4: // jmp
				{ const char* jp = getenv("TIO_JLOG"); if (jp) {
				    fprintf(stderr, "JMP lazy=%d mod6=%d head=%d width=%ju\n",
				            d->memptr->val ? 0 : 1, mod(d,6), (int)d->head, d->width); } }
				if (!d->memptr->val) {
			...
			case 39: // rot
				if (!d->memptr->val) {
					if (getenv("TIO_JLOG")) fprintf(stderr, "LAZY-ROT mod6=%d head=%d width=%ju\n", mod(d,6), (int)d->head, d->width);
				...
				{ static long n39 = 0; n39++;
				  if (getenv("TIO_VLOG")) fprintf(stderr, "R %ld %d %d\n", n39, (int)a->head, mod(a,59049)); }
				break;
			case 40: // movd
				if (!d->memptr->val) {
				...
				if (d->width > max_wordwidth) {
					...
					if (w > max_wordwidth) {
						max_wordwidth = w;
						{ uintmax_t would = det_growth_policy(max_wordwidth, rotwidth, growth_step, growth_slack);
						  if (would != rotwidth) fprintf(stderr, "[W-HACK] step=%d wordwidth=%ju would grow rotwidth %ju -> %ju (kept 20)\n", step, max_wordwidth, rotwidth, would); }
					}
				}
				break;
			case 62: // opr
				...
				if (!d->memptr->val) {
					if (getenv("TIO_JLOG")) { Number* fv = initial_values[mod(d,6)];
					  fprintf(stderr, "LAZY-OPR n62=%ld mod6=%d head=%d fillval_unicode=%d\n", n62, mod(d,6), (int)d->head, fv->unicode); }
					d->memptr->val = clone_number(initial_values[mod(d,6)]);
				}else{
					repair_number_after_xlat2(d->memptr->val);
				}
				opr(a,d->memptr->val);
				if (getenv("TIO_VLOG")) fprintf(stderr, "C %ld %d %d\n", n62, (int)a->head, mod(a,59049));
				}
				break;
			case 81: // hlt
				return 0;
