● Python scripts:
  ...
  - mb_gen.py — Generates straight-line INPUT/PRINT programs; used to build claude_printer.mu
  ...
  Malbolge Unshackled programs (.mu):
  - claude_printer.mu — 41-byte straight-line printer; reads "Claude is the best!\n" from stdin and echoes it
  - claude_noInput.mu — 3724-byte no-input printer; run with fast20 claude_noInput.mu < /dev/null
  - claude_crz1.mu — Reads stdin, applies one CRZ transform per character, prints result
  - claude_crz2.mu — Same but two CRZ transforms per character
  - hello_printer.mu — 35-byte stdin printer for "Hello, Malbolge!\n" (same technique as claude_printer)
  - branch_flag.mu — 300-byte flag-idiom branch program; input 'Y' prints 'R', input 'N' prints '%'
